...
+ **直接调用数据不显示的bug**
  + 百度搜索关键词： vue props更新 子组件重新渲染过程
  + https://segmentfault.com/a/1190000021853790
...
上面代码会报错，可以在这里打印1，在子组件的created和mounted打印23，再去子组件的getDepartmentDetail函数里面打印4
就会发现，这里调用子组件的时候，我的currentNodeId还没有传递进去，子组件的props还是之前的
旧数据null，不是我们这个设置的id。因为父组件的currentNodeId更新了，子组件更新以后我们没有拿到
