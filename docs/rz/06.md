 ...
 <el-table-column align="center" label="操作">
    <template v-slot="{ row }">
        <template v-if="row.isEdit">
            <!-- 编辑状态 -->
            <el-button type="primary" size="mini" @click="editInLine">确定</el-button>
            <el-button size="mini" row.isEdit = false>取消</el-button>
        </template>
        <template v-else>
              <!-- 非编辑状态 -->
            <el-button size="mini" type="text">分配权限</el-button>
...
- 点击编辑按钮时缓存数据-代码位置(**src/views/role/index.vue**)
```javascript
// 点击编辑按钮的时候，
btnEditRow(row) {
  row.isEdit = !row.isEdit
  // 将当前的值 深拷贝一下，就是说我们 数组里面的 对象数据 row不去修改它
  // 而是将这个值进行拷贝，编辑的时候，input上面绑定的值也都用这一些变量
  // this.currentItem = cloneDeep(row)
  this.currentItem = { ...row }
},
    
修改模板上面关于input的绑定 ---
<el-input v-if="row.isEdit" v-model="currentItem.name" /> 
<el-switch v-if="row.isEdit" v-model="currentItem.state" />
<el-input v-if="row.isEdit" v-model="currentItem.description" />
```

- 点击编辑时先将其他的行利用排他思想改为true-代码位置(**src/views/role/index.vue**)
```javascript
btnEditRow(row) {
...
// 点击确定时触发
async  btnEditOK(row) {
  if (this.currentItem.name && this.currentItem.description) {
    // 下一步操作
    await updateRole({ ...this.currentItem, id: row.id })
    row.isEdit = false
    // 更新成功
...
methods: {
  async getDepartment() {
    ...
    this.depts = transfromListToTree(res.data, 0)
  }
}
```
## 7.员工管理-选中首个节点
