...
父组件里面使用
<h4>{{ count }}</h4>
<TestSync v-model="count" />
上面代码等价于 <TestSync :value="count" @input="count = $event" />

子组件里面
props: ['value']
方法里面： this.$emits('input', '修改数据')
```
...
上面代码等价于 <Child :name='name' @update:name='name=$event'>
```

总结：

+ v-model 是 value属性 和 input事件的绑定， 一个组件上面只能使用一个v-model
+ name被sync修饰以后，就是 name属性 和 update:name 事件的绑定，
+ 同一个组件上面可以使用多个sync修饰符
+ 他们可以简化父子通信的代码书写，提高代码可读性
